...
pub fn build(b: *std.Build) !void {
    ...
    const sdl_ogc = b.dependency("sdl_ogc", .{});
    ...
    // Added src/SDL_config.h which is the config provided by SDL after you run:
    // cmake -S. -Bbuild -DCMAKE_TOOLCHAIN_FILE="$DEVKITPRO/cmake/Wii.cmake" -DCMAKE_BUILD_TYPE=Release -Wno-dev >/dev/null
    // See: https://github.com/devkitPro/SDL/blob/9a759681abf8883cdbaa9a0299c07d13f08556d1/docs/README-ogc.md
    //
    // Should ideally do it properly as per "SDL_config.h.cmake" logic below but not doing this effort for now.
    const config_header = b.addConfigHeader(.{
        .style = .{ .cmake = b.path("src/SDL_config.h") },
        .include_path = "SDL_config.h",
    }, .{});
    lib.addConfigHeader(config_header);
    lib.installConfigHeader(config_header);
    // const config_header = b.addConfigHeader(.{
    //     .style = .{ .cmake = sdl_ogc.path("include/SDL_config.h.cmake") },
    ...
    //     .HAVE_STDARG_H = true,
    //     .HAVE_STDDEF_H = true,
    //     .HAVE_MALLOC = true, // fix "use of undeclared identifier 'PTHREAD_MUTEX_INITIALIZER'"
    //     .HAVE_STRING_H = true, // fix "strerror" missing
    //     .SDL_AUDIO_DRIVER_OGC = true,
    //     .SDL_FILESYSTEM_OGC = true,
    //     .SDL_JOYSTICK_OGC = true,
    //     .SDL_THREAD_OGC = true,
    //     .SDL_TIMER_OGC = true,
    //     .SDL_VIDEO_DRIVER_OGC = true,
    //     .SDL_VIDEO_RENDER_OGC = true,
    //     .SDL_LOADSO_DISABLED = true,
    // });
    // lib.addConfigHeader(config_header);
    // lib.installConfigHeader(config_header);

    // add SDL2 *.h files
    lib.addIncludePath(sdl_ogc.path("include"));

    // Add SDL2 *.c files
    for (generic_src_files) |src_file| {
        lib.addCSourceFile(.{
            .file = sdl_ogc.path(src_file),
        });
    }
    for (ogc_src_files) |src_file| {
        lib.addCSourceFile(.{ .file = sdl_ogc.path(src_file) });
    }
    lib.linkSystemLibrary("aesnd");
    lib.linkSystemLibrary("fat");
    lib.linkSystemLibrary("wiiuse");
    lib.linkSystemLibrary("bte");
    lib.linkSystemLibrary("wiikeyboard");
    ...
    module.addIncludePath(sdl_ogc.path("src")); // allow us to access "video/ogc/SDL_ogcevents_c.h"
    module.addIncludePath(sdl_ogc.path("include"));
}
...
const ogc_src_files = [_][]const u8{
    ...
    "src/thread/ogc/SDL_sysmutex.c",
    "src/thread/ogc/SDL_syssem.c",
